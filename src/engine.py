# src/engine.py
"""
Core algorithmic logic for the Graph R-Learner experiments.
Contains all functions for training, cross-fitting, and CATE estimation.
This version correctly handles separated model and training kwargs.
"""
import torch
...
def estimate_cate_gnn(Y_res, T_res, X, edge_index, model_obj=None, model_kwargs={}, training_kwargs={}):
    lr = training_kwargs.get('lr', 1e-3)
    epochs = training_kwargs.get('cate_epochs', 200)
...
def estimate_cate_tlearner_gnn(X, T, Y, edge_index, model_kwargs={}, training_kwargs={}):
    # Use nuisance_epochs for T-Learner as it's a single-stage model
    epochs = training_kwargs.get('nuisance_epochs', 150) 
    model = T_Learner_GNN(X.shape[1], **model_kwargs)
    train_idx = torch.arange(len(X))
    model = train_gnn_model(model, X, Y.squeeze(), edge_index, train_idx, is_binary=False, T=T, epochs=epochs, **training_kwargs)
